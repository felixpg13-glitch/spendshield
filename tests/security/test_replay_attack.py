...
def test_replay_same_key_denied():
    ...
    r1 = g.authorize("bot", 20, "amazon.com", meta=meta)
    r2 = g.authorize("bot", 20, "amazon.com", meta=meta)   # 重放
    assert r1.decision == "ALLOW"
    assert r2.decision == "DENY"
    assert "replay" in r2.reason.lower()
    assert g._v2_estate.spent_total == 20, "重放不应二次记账"
...
def test_same_request_different_key_allowed():
    ...
    g = _make_guard()
    r1 = g.authorize("bot", 20, "amazon.com", meta={"idempotency_key": "a"})
    r2 = g.authorize("bot", 20, "amazon.com", meta={"idempotency_key": "b"})
    assert r1.decision == "ALLOW"
...
def test_replay_different_amount_same_key_denied():
    ...
    g = _make_guard()
    r1 = g.authorize("bot", 20, "amazon.com", meta={"idempotency_key": "k1"})
    r2 = g.authorize("bot", 40, "amazon.com", meta={"idempotency_key": "k1"})
    assert r1.decision == "ALLOW"
    assert r2.decision == "DENY"
